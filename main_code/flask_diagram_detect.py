...
def diagram_detection_logic(image_data ,cropped_ocr_result_json_file_name):
    # 실제 화살표 탐지 로직이 여기에 위치합니다.
    # 예를 들어, 이미지 처리 후 결과 반환
    img = np.array(image_data)

    contours, hierarchy = cv2.findContours(preprocess(img), cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE)
    #밑에껀 컨투어의 꼭짓점 좌표만
    ...
    #open_path = "jsonFile/Sep_test_diagram"+image_index+".json"
    open_path = "./ocrResult/{}".format(cropped_ocr_result_json_file_name)
    #test_diagram
    #compare_image
    ...
    #return {"result_sentence": sentence_result}
    return sentence_result


#@app.route('/diagram_detect', methods=['POST'])
def main_diagram_detect(image_data, cropped_ocr_result_json_file_name):
    #image_data = request.files['image'].read()
    print("")
    print("main_diagram_detect_cropped_ocr_result_json_file_name : {}".format(cropped_ocr_result_json_file_name))
    result = diagram_detection_logic(image_data, cropped_ocr_result_json_file_name)
    return result
